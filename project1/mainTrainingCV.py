...
#==============================================================================
# Generate new train and test indices for cross validation
#nFiles = len(os.listdir(folder))
#cvFold = 10
#nItemPerFold = int(math.floor(nFiles/cvFold))
#randPermInd = np.random.permutation(nFiles)
#trainInd = randPermInd[0:nItemPerFold*9]
#testInd  = randPermInd[nItemPerFold*9:]


# Checking if indices are split correctly
# list1 = np.concatenate((trainInd,testInd),axis=0)
# list1.sort()
# print(list1)
# print(len(list1))
#==============================================================================

trainInd=np.load('trainInd.npy')
trainInd=trainInd[trainInd<46]

colorSpace=cv2.COLOR_RGB2HSV
trainingPixels = np.concatenate(trainingPixels_stored[trainInd]) # for cross-validation
#trainingPixels = np.concatenate(trainingPixels_stored[:]) # for full training
