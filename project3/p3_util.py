...
#	for i in xrange(200,len(lidar_data),10):
    for i in xrange(0,1):
        for (k,v) in enumerate(lidar_data[i]['scan'][0]): # (k,v) = (idx,val)
            if v > 30: # if sensor value > 30, set it to 0 (too far from sensor)
...
def plot_pose(lidar_data):
    x = [l['pose'][0,0] for l in lidar_data]
    y = [l['pose'][0,1] for l in lidar_data]
    theta = [l['pose'][0,2] for l in lidar_data]
